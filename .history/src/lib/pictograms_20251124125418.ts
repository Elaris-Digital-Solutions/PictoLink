    ...
    const pictograms = await Promise.all(
      ids.slice(0, 10).map(async (id) => {
        ...
        return {
          ...
          image_urls: {
            svg_color: data.image_urls?.svg_color || data.svgColor || `https://static.arasaac.org/pictograms/${id}/${id}_2000.svg`,
            png_color: data.image_urls?.png_color || data.pngColor || `https://static.arasaac.org/pictograms/${id}/${id}_500.png`,
